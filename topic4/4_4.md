...
### Expanded Commands for IPv4 Address Configuration

#### **1. `ip addr add` (Manually Assign an IPv4 Address)**:
  - **Purpose**: Assign a specific IPv4 address to a network interface manually.
  - **Syntax**:  
    ```bash
    sudo ip addr add <IP_address>/<subnet_mask> dev <interface_name>
    ```
    - `<IP_address>`: The IPv4 address you want to assign.
    - `<subnet_mask>`: The subnet mask in CIDR notation (e.g., `/24` for a 255.255.255.0 mask).
...
#### **2. `ip addr show` (Verify IPv4 Address Assignment)**:
  - **Purpose**: Display the current configuration of all network interfaces, including assigned IP addresses.
  - **Syntax**:  
    ```bash
    ip addr show
    ```
...
#### **3. `ping` (Verify Network Connectivity)**:
